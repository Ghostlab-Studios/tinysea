...
- `EcosystemSimulator` — 10-step biology sequence (v12-per-species-tracking): thermal performance (per-species `TempMultiplier` scales the deviation from the run base temp before the curve: `dampedTemp = temp + (temp - BaseTemperatureC) * (TempMultiplier - 1)`, V1; default 1.0 is bit-identical) → feeding/predation (Tier 1 FedRate from food-pool density + Tier 2 Holling II with **per-predator FedRate**, v11) → raw final performance → update Condition (rates scaled by Pmax; Day-0 `Condition` seeded from per-species `InitialCondition`, default 1.0, V1; per-branch overshoot guard clamps the Euler move so it cannot cross the target — `max(target,..)` on drain, `min(target,..)` on recovery — letting drain/recovery rates exceed 1.0 for memoryless no-memory runs, V1) → final performance → thermal death → condition death → reproduction (Pmax × condition-driven; no soft cap on births) → natural death → population rounding. Uses accumulators (birth, predation, natural death, condition death) for fractional population tracking. **Carrying capacity is always on** as of v11.1 — no toggle. Rationale (per `EcosystemSimulator.cs:121-125`): Tier 1 species without a resource ceiling grow without bound, which is biologically meaningless and triggered integer-overflow in birth accumulators around day ~50 of any cap-off scenario. **Per-species event counters added in v12** (`LastBirthsBySpecies`, `LastTempDeathsBySpecies`, etc.) keyed by `FullName`, alongside accumulator accessors (`GetBirthAccum(fullName)` etc.). Rationale: research analysts need to track each species individually, not have them all roll up into a single fixed-variant bucket. (As of the T1 refactor the tier rollup is dynamic per `variantLabel` — see "CSV output formats" below — so the old fixed `Tier1Custom`/`Tier2Custom` buckets no longer exist.) Crash detection: `totalPop == 0`. See [`docs/simulation-spec.md`](./docs/simulation-spec.md) for the authoritative spec.
- `TemperatureCalculator` — Standalone temperature model (not the game's `Temperature` class). Daily AR(1) autocorrelation is now a parameter, `AutocorrelationCoefficient` (default 0.7 = legacy `0.7*yesterday + 0.3*new`, 0 = white noise, range 0..1, V1): `variation = prev*coeff + newRandom*(1-coeff)`; echoed in the scenario CSV config header as `#config:autocorrelation_coefficient`
...
**CSV output formats:**
- **Scenario CSV** — Each scenario file has `#config:key,value` lines (simulation params), then a `#species:` table (header + data rows with `#species:` prefix), then the daily step data. All `#` lines are ignored by R's `read.csv()`. **Tier-variant rollup is dynamic** (`SimulationRunner.BuildVariantRollupColumns`): one `Tier{n}_{variantLabel}` column per distinct (tier, variant label) present in the run (e.g. `Tier1_Hot_Specialist`, `Tier1_M2`), sanitized via `SanitizeColumnName`, ordered by (tier asc, label asc). The legacy fixed 4-bucket `Tier1Arctic/Common/Tropical/Custom` (+`Tier2*`) columns and the `ThermalVariant` enum names are **no longer emitted anywhere in the CSV**. **v12: 17 per-species columns appended** to each daily row, ordered by `(Tier asc, FullName asc)`. Tier-rollup invariant: per-species values sum to the matching `Tier{n}_{label}` column, which sum to the `Tier{n}Pop` total.
