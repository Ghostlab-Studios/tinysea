...
| `AutocorrelationCoefficient` | `float` | `0.7f` (`:106`) | absent, defaults to `0.7` | `[Range(0f, 1f)]` (`:105`) | dimensionless [0,1] | `AutocorrelationCoefficient` (`:370`) | The AR(1) coefficient (phi) for daily temperature variation. Used only when `Autocorrelated` is true: `variation = previousDay * coeff + newRandom * (1 - coeff)` (`TemperatureCalculator.cs:188`). Replaces the previously hardcoded 0.7/0.3 blend; the default 0.7 reproduces it exactly. 0 is white noise; as the coefficient approaches 1 the daily amplitude shrinks toward zero (red-noise dampening). It has no effect while `Autocorrelated` is false. Emitted in the per-scenario CSV config header as `#config:autocorrelation_coefficient` (`SimulationRunner.cs:771`). |
...
| `tempMultiplier` | `float` | `1.0f` (`:58`) | `>= 0` (bulk CSV rejects negatives; UI blocks Run) | dimensionless multiplier | Per-species multiplier on the experienced deviation from the run base temperature, applied in Step 1 before the thermal curve: `dampedTemp = temperature + (temperature - BaseTemperatureC) * (tempMultiplier - 1)`, then `CalculatePerformance(dampedTemp)` (`EcosystemSimulator.cs:607-608`). `1.0` = no change (bit-identical), below 1 dampens the swing (thermal inertia), above 1 amplifies. The per-species `TemperatureDebuff` is still added inside `CalculatePerformance`, after this damping. Maps to `SimSpecies.TempMultiplier`. |
| `initialCondition` | `float` | `1.0f` (`:69`) | `0..1` | dimensionless Condition | Day-0 value of `Condition` (the [0,1] health/energy reserve), copied into `Condition` at scenario start (`EcosystemSimulator.cs:384-385`). Replaces the previously hardcoded 1.0. `1.0` = fully charged; lower starts the organism nearer a midpoint. Maps to `SimSpecies.InitialCondition`. |
