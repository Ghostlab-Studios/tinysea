...
food_density = max(0, 1 - tier1Pop / capSafe)         // 1 = empty pool, 0 = pool at/over capacity
```

The order of operations within the day is defined by the 10-step biology sequence in `biology-and-formulas.md` (Step 2 is Feeding). This document only fixes which population snapshot feeds the density; see that document for how `food_density` then drives Tier 1 `FedRate` and the rest of the step.
...
| `eatingAmount` | `float` | `0f` | none | prey individuals consumed per creature per biology step | Prey consumed per predator. Maps to `SimSpecies.EatingAmount`. Tier 1 ignores it (Tier 1 feeds from the resource pool, not by eating). |
...
After the skeleton, a Tier 1 no-predator penalty multiplies `births` by `NO_PREDATOR_PENALTY = 0.85` when the total Tier 2 population is below the alive floor of 1.0 (`SimSpecies.cs:55`, `EcosystemSimulator.cs:1207-1215`). In the shipped Tier 1 only run there are no Tier 2 species, so this 0.85 factor always applies to every Tier 1 species. Births below 2 population do not occur at all: `ApplyReproduction` returns early when `Population < MIN_POPULATION_FOR_REPRODUCTION = 2.0` (`EcosystemSimulator.cs:274, 1152`). Fractional `births` accumulate in a per-species birth accumulator; only the whole-number floor is added to the population each step and the remainder carries to the next step (`EcosystemSimulator.cs:1228-1238`).
...
### 4.5 Hunting efficiency (Tier 2 legacy; Tier 1 resource extraction)

These two fields drive the legacy Tier 2 Holling II hunting and, by convention, the Tier 1 resource extraction efficiency. For Tier 1 the value used in the shipped data is `1.0` (perfect extraction). See `biology-and-formulas.md` for the full per-step use; the Tier 1 chain is summarized here so the formula is in one place.

Tier 1 feeding chain (the only feeding path in the shipping Tier 1 build). For each Tier 1 species with population at or above the alive floor, the per-step feeding satisfaction `FedRate` is computed as:

```
FedRate = min(1, huntingEfficiency * food_density)        // EcosystemSimulator.cs:769
```

where `food_density = max(0, 1 - tier1Pop / capSafe)` is the shared-pool density defined in section 2.2 (sampled at the start of Step 2). `FedRate` is a dimensionless multiplier in [0,1], not a count of consumed individuals: 1 means fully fed, 0 means starving. It feeds Step 3 (`RawFinalPerformance = RawThermalPerformance * FedRate`, `:608`) and through that the Condition update in Step 4. `huntingEfficiency` is the Tier 1 resource-extraction efficiency (the linear slope of `FedRate` in `food_density`); at the shipped value `1.0`, `FedRate` equals `food_density` directly. Tier 1 species below the alive floor get `FedRate = 0` (`:775`).

`eatingAmount` plays no role for Tier 1. It is read only on the Tier 2 predator demand path (`rawDemand = Population * EatingAmount * ThermalPerformance * BiologyStep`, `:814`); the Tier 1 block never references it (`:759-781`). Likewise `huntingVariance` is applied to Tier 2 only (`:809`); Tier 1 `FedRate` has no random term.
...
| Field | Type | C# default | Range/validation | Units | Meaning |
|---|---|---|---|---|---|
| `huntingEfficiency` | `float` | `0.75f` (`:78`) | none | fraction (extraction/success rate) | Tier 2 base hunting success (0.75 = 75%). For Tier 1 it is the linear resource extraction efficiency in `FedRate = min(1, huntingEfficiency * food_density)` (`EcosystemSimulator.cs:769`); shipped Tier 1 data uses `1.0`. Maps to `SimSpecies.HuntingEfficiency`. |
| `huntingVariance` | `float` | `0.15f` (`:80`) | none | fraction (+/- range) | Random variance around hunting success (0.15 = +/- 15%). Applied to Tier 2 only (`:809`); Tier 1 ignores it. Maps to `SimSpecies.HuntingVariance`. |
