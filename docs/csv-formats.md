...
```
#config:model_version,v11-per-predator-fedrate
...
`model_version` is the first line so downstream tooling can identify which simulator produced the file at a glance. `v11-per-predator-fedrate` corresponds to the spec changes in [`simulation-spec.md`](./simulation-spec.md) §v11 changes (which builds on v10's food-pool reframe).
...
# Model Version,v11-per-predator-fedrate
# Total Runs,<n>
# Generated,<yyyy-MM-dd HH:mm:ss>
