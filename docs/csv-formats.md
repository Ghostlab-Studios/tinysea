...
```
#config:model_version,v10-food-pool
...
`model_version` is the first line so downstream tooling can identify which simulator produced the file at a glance. v10-food-pool corresponds to the spec changes in [`simulation-spec.md`](./simulation-spec.md) §v10 changes.
