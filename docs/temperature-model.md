...
| `MinTemp` | float | -5 (line 27) | Hard floor applied after the sum, degrees C | `TemperatureBoundsMin` (-5, `SimulationConfig.cs:112`) | `TempMin` |
| `MaxTemp` | float | 40 (line 28) | Hard ceiling applied after the sum, degrees C | `TemperatureBoundsMax` (50, `SimulationConfig.cs:115`) | `TempMax` |

Notes on the defaults:

- The class field defaults and the `SimulationConfig` field defaults agree for every parameter except `MaxTemp`. The class default is 40 (`TemperatureCalculator.cs:28`); the `SimulationConfig` default is 50 (`SimulationConfig.cs:115`). In practice the caller always overwrites the class field, so the active ceiling is whatever the config or bulk row supplies. The class field default of 40 only takes effect if a `TemperatureCalculator` is constructed and used without a caller copying config onto it.
...
Then the smoothing (`TemperatureCalculator.cs:183-194`):

```
if UseAutocorrelation:
    variation = _previousDayVariation * 0.7 + newRandom * 0.3   // 70% yesterday, 30% new
else:
    variation = newRandom
...
The calculator's parameters are echoed verbatim into each scenario CSV as `#config:` comment lines, read straight off the live `TempCalc` instance (`SimulationRunner.cs:755-764`): `base_temperature`, `seasonal_amplitude`, `climate_trend_per_year`, `variability_magnitude`, `warming_bias`, `daily_variation_range` (from `BaseRandomness`), `randomness_growth_rate`, `autocorrelated`, `temperature_bounds_min`, and `temperature_bounds_max`. The realized daily temperature appears per row, and per scenario `AvgTemperature`, `MinTemperature`, and `MaxTemperature` are summarized into `ScenarioResult` (`SimulationRunner.cs:972-974`). The exact CSV layout is documented in `csv-output-formats.md`; the configuration echo is documented in `configuration-reference.md`.
...
3. For day index `d` (zero based):
   ...
   - `daily`: `r = BaseRandomness + RandomnessGrowthRate * floor(d / 365)` (INTEGER year, stepwise growth); `newRandom = (rng.NextDouble()*2 - 1) * r`; if autocorrelation on `variation = prev*0.7 + newRandom*0.3` else `variation = newRandom`; store `variation` as the new `prev`; use `variation`.
